    ...
    while True:
                    ...
                    try:
                        ...
                        if ver_id:
                            ...
                            run_cmd(["python","-m","core.ml.backtests.build_plans_from_signals",
                                     ...
                                     "--from",f_inf,"--to",t_inf])
                            # 3b) Toma de decisiones (opcional) y ejecución training
                            try:
                                # Procesar señales recientes y generar planes vía Decision Maker
                                ...
                                if execution_results['successful'] > 0:
                                    ...
                                    print(f"[{symbol}] ✅ Ejecutados: {execution_results['successful']} trades")
                                    print(f"[{symbol}] 💰 PnL del símbolo: ${symbol_pnl:.2f}")
                                    print(f"[{symbol}] 🏦 PnL total portfolio: ${total_pnl:.2f}")
                                    logging.info(
                                        f"EXECUTION_SUMMARY: {symbol} | trades={execution_results['successful']} "
                                        f"| symbol_pnl={symbol_pnl:.2f} | total_pnl={total_pnl:.2f}"
                                    )
                            except Exception as e:
                                logging.warning("Fallo en decisiones/ejecución para %s: %s", symbol, e)
        ...
        if args.once:
            ...
            try:
                ...
                print(f"💰 Balance inicial: ${total_balance['total_initial']:,.2f}")
                print(f"💰 Balance actual: ${total_balance['total_current']:,.2f}")
                print(f"📈 PnL total: ${total_balance['total_pnl']:,.2f} ({total_balance['total_pnl_pct']:.2f}%)")
                print(f"🎯 Progreso hacia objetivo: {total_balance['progress_to_target']*100:.1f}%")
