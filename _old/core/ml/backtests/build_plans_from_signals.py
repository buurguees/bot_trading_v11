...
def fetch_signals(conn, symbol, tf, ver_id, ts_from, ts_to):
    """Lee señales asegurando trazabilidad al modelo.
    - Usa agentsignals en minúsculas
    - Filtra por agent_version_id si existe, si no por meta->>'direction_ver_id'
    """
    q = text("""
        SELECT timestamp, side, strength,
               COALESCE(agent_version_id, (meta->>'direction_ver_id')::int) AS agent_version_id
        FROM trading.agentsignals
        WHERE symbol=:s AND timeframe=:tf
          AND COALESCE(agent_version_id, (meta->>'direction_ver_id')::int) = :vid
...
def insert_plan(conn, *, bar_ts, symbol, tf, side, entry, sl, tp,
    ...
    # Con ON CONFLICT DO NOTHING, scalar() puede retornar None si no se insertó
    ...
    with eng.begin() as conn:
        ...
        for row in sig.itertuples(index=False):
            ...
            if signature:
                mem_q = text("""
                  WITH best AS (
                    SELECT win_rate, avg_pnl, sharpe, n_trades, mode,
                           ROW_NUMBER() OVER (PARTITION BY symbol, timeframe, signature
                                              ORDER BY CASE WHEN mode='real' THEN 1 ELSE 2 END) AS rn
