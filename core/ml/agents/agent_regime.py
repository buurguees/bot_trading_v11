...
=============
Lee:
  - config/train/training.yaml (encoder + heads.regime.query_tf si existe)
  - config/market/symbols.yaml
  - market.features
Escribe:
  - ml.agent_preds (UPSERT) con task='regime' (pred_label trend|range)
    y JSON 'probs' con claves trend, range, vol_q1..vol_q4, vol_bucket.

Funciones:
- run_once(model_name, model_version): igual patrón que el agente direction.
"""
...
def run_once(model_name: str = "regime_v1", model_version: str = "dev") -> int:
    ...
    qtf = heads_cfg.get("regime", {}).get("query_tf", enc_cfg.query_tf_default)
    ...
    for sym in symbols:
        ...
        _upsert_agent_pred(
            engine,
            symbol=sym, timeframe=qtf, ts=ts, task="regime",
            label=pred["label"], probs=pred["probs"],
            model_name=model_name, model_version=model_version
        )
        written += 1
        logger.info(f"[{sym}] regime={pred['label']} conf={pred['probs'][pred['label']]:.3f} ts={ts}")
