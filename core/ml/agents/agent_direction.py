...
================
Lee:
  - config/train/training.yaml (parámetros del encoder, features, query_tf por head si existe)
  - config/market/symbols.yaml (lista de símbolos)
  - market.features (para recuperar ts de última vela cerrada)
Escribe:
  - ml.agent_preds (UPSERT) con task='direction'

Funciones:
- run_once(model_name, model_version):
    Itera símbolos, calcula predicción (última vela cerrada del TF elegido),
    y upserta en ml.agent_preds.

Notas:
- El TF para la inferencia se toma de training.yaml -> heads.direction.query_tf
  y si no existe, usa encoder.query_tf_default.
"""
...
def run_once(model_name: str = "direction_v1", model_version: str = "dev") -> int:
    ...
    qtf = heads_cfg.get("direction", {}).get("query_tf", enc_cfg.query_tf_default)
    ...
    for sym in symbols:
        ...
        _upsert_agent_pred(
            engine,
            symbol=sym, timeframe=qtf, ts=ts, task="direction",
            label=pred["label"], probs=pred["probs"],
            model_name=model_name, model_version=model_version
        )
        written += 1
        logger.info(f"[{sym}] direction={pred['label']} conf={pred['probs'][pred['label']]:.3f} ts={ts}")
